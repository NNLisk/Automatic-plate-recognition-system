...
def show_contours_debug():
    rects = thresholded_2_segmented_letters(
        "data/inference/sessions/4/thresholded.jpg",
        "data/inference/sessions/4"
    )

    img = cv2.imread("data/inference/sessions/4/cropped.jpg", cv2.IMREAD_GRAYSCALE)
    img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)

    for i, (x, y, w, h) in enumerate(rects):
        print(x,y, w*h, w/float(h))
